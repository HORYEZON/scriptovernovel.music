...
export const openApiSpec: OpenAPIV3.Document = {
    ...
    schemas: {
      ...
      Event: {
        ...
        properties: {
          ...
          lineup: { type: "string", nullable: true, description: "One act per line." },
          latitude: { type: "number" },
          longitude: { type: "number" },
          eventDate: { type: "string", format: "date-time", nullable: true, description: "Null = TBA." },
          ticketUrl: { type: "string", format: "uri", nullable: true },
          ticketPrice: { type: "number", nullable: true, description: "PHP." },
          ticketNote: { type: "string", nullable: true },
          status: { type: "string", enum: ["SCHEDULED", "SOLD_OUT", "CANCELLED", "POSTPONED"] },
  ...
  paths: {
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  city: { type: "string", nullable: true, maxLength: 80, description: "Shown beside the venue and used for the MusicEvent address." },
                  lineup: { type: "string", nullable: true, maxLength: 600, description: "The rest of the bill, one act per line." },
                  latitude: { type: "number" },
                  longitude: { type: "number" },
                  eventDate: { type: "string", format: "date-time", nullable: true, description: "Null = a TBA show, which still lists as upcoming." },
                  ticketUrl: { type: "string", format: "uri", nullable: true, description: "http(s) only — rendered as a link on /shows." },
                  ticketPrice: { type: "number", nullable: true, minimum: 0, maximum: 100000, description: "PHP. Null is not 'free' — use ticketNote for that." },
                  ticketNote: { type: "string", nullable: true, maxLength: 120, description: "Shown instead of ticketPrice when set, e.g. \"₱250 at the door\"." },
                  status: { type: "string", enum: ["SCHEDULED", "SOLD_OUT", "CANCELLED", "POSTPONED"], default: "SCHEDULED" },
                  enabled: { type: "boolean" },
                },
              },
            },
    ...
    "/events/public": {
      get: {
        tags: ["Events"],
        summary: "List enabled events (public — the /shows list and its map)",
        description:
          "Every enabled, non-trashed event with the gig fields /shows renders: city, lineup, ticket link/price/note and status. Dates are ISO strings; media is pre-sorted by `order`.",
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  lineup: { type: "string", nullable: true, maxLength: 600 },
                  latitude: { type: "number" },
                  longitude: { type: "number" },
                  eventDate: { type: "string", format: "date-time", nullable: true },
                  ticketUrl: { type: "string", format: "uri", nullable: true },
                  ticketPrice: { type: "number", nullable: true, minimum: 0, maximum: 100000 },
                  ticketNote: { type: "string", nullable: true, maxLength: 120 },
                  status: { type: "string", enum: ["SCHEDULED", "SOLD_OUT", "CANCELLED", "POSTPONED"] },
                  isNextEvent: { type: "boolean" },
                  enabled: { type: "boolean" },
                  displayOrder: { type: "integer" },
              ...
              schema: {
                ...
                properties: {
                  ...
                  modelUrl: {
                    ...
                    nullable: true,
                    description:
                      "For a custom .glb prop, the model URL. For a divider wall (kind `divider`) it carries that panel's config JSON — width/height/thickness in metres, colour, an optional tiled surface texture and whether it is mirrored, and whether visitors are blocked by it. For the kind-marked config singletons this column carries JSON instead: the About plaque/label config, the Stories Room's `story-podium-model` (uploaded pedestal .glb, its tiled texture and book height), the Arcade Room's `arcade-config` (room-wide CABINET/POSTER default, plus the cabinet .glb or tiled texture and the screen height/depth the game screen and marquee are drawn at), the About room's `about-contact` (the Contact Desk's optional .glb or tiled texture, plus the title/subtitle on the Send an Email panel it opens and the words on its walk-up prompt), and `room-banner` — provisioned once for each of the Stories/Arcade/Services/About/Cosplay rooms, holding the one plaque style every label that room draws reads from (panel/edge/text colours, edge width, font and text scale, an optional stretched panel texture, panel brightness, and a frosted-glass mode with its own opacity plus an optional shimmer sweep and its speed/strength). The Vinyl Room's `vinyl-room-config` is the same idea: the deck (an optional turntable .glb and its room-local placement), the effect amounts a record starts from (reverb/lo-fi/crackle/delay, the shoegaze board's phaser/flanger/chorus/vibrato/tremolo and the shared sweep speed, whether it starts with backward play (`reverse`) or backmasked (`backmask`, plus its window length — the two are mutually exclusive), plus rpm and fine speed), and the Lyrics Wall — on/off, which wall, the lyric and glow colours, the font and text scale, the same full panel finish a `room-banner` carries (panel/edge colours, edge width, stretched texture, brightness, glass mode with its opacity, shimmer with its speed/strength), and an optional video behind the lyrics (`videoSource` none/upload/youtube, the uploaded clip URL and/or YouTube link, its brightness, and whether it is muted).",
                  },
                  hidden: {
                    type: "boolean",
                    description:
