...
- **Lyrics pages** — `/music/[slug]/[track]`: one song's words on their own indexable page (`"<song> lyrics"` is a phrase people type, and nothing else on the site answered it). Title is `"<Song> — lyrics"`, the cover is the share image, and it carries `MusicRecording` JSON-LD with the lyrics attached plus a `BreadcrumbList`. Section headers in `[brackets]` are kept for reading and never timed. Prev/next through the tracklist and the rest of the record underneath, so a page arrived at from a search isn't a dead end. **When a track has been tap-synced** (below) *and* the record has a published vinyl, the page plays that audio and lights each line as it comes — seeking to the song's first line rather than the top of the record. Resolves a track **slug or number**, since `ReleaseTrack.slug` is nullable for older rows; `trackLyricsHref` in [`lib/lyrics.ts`](lib/lyrics.ts) owns that fallback
- **Release pages** — `/music/[slug]`: each record on its own indexable, shareable page — the cover, the player, the tracklist, every platform it's on, the videos made for it, a link into the Vinyl Room when it exists as a record there, and the rest of the discography. Its own `<title>`/description/OG image (the cover), a canonical on the slug, and `MusicAlbum` + `BreadcrumbList` JSON-LD with the tracklist as `MusicRecording`s. `/music` is the browsing view and each card's title links here. **The route resolves by slug *or* id** — `Release.slug` is nullable (generated on create), so every link on the site is `slug ?? id` via [`releaseHref`](lib/releases.ts) and [`getPublicReleaseBySlug`](lib/releases-server.ts) looks up both. The cover tile + lightbox ([`ReleaseCover.tsx`](components/public/ReleaseCover.tsx)) and the tracklist with its lyrics disclosure ([`ReleaseTracklist.tsx`](components/public/ReleaseTracklist.tsx)) are shared with the `/music` card rather than copied
...
- **Press kit / EPK** — `/press`: what a booker, a blog or a festival asks for, on one page they can read, copy from, download from and print. A short bio *written as a short bio* (with a word count and a Copy button, beside the long one), press quotes, the line-up, press photos at full size with the credit attached (`download` on the anchor, so a click saves the file), the logo, the discography with every platform link, video, selected shows, awards, the tech rider with a Copy button, a stage plot, and the booking address. **The "one-sheet" is this page printed**: a `@media print` block in `globals.css` turns it into ink-on-paper (chrome, embeds and buttons dropped via `data-print="hide"`, link destinations printed after each link) and the browser's own dialog saves it as a PDF — the same route the order receipt takes rather than generating one server-side. Gathered by [`lib/press-server.ts`](lib/press-server.ts); the press-only fields and the quote sanitizer are in [`lib/press.ts`](lib/press.ts). Carries `MusicGroup` JSON-LD with a booking `contactPoint`
- **Coming soon & stock alerts** — a product can be **announced before it's on sale** (`Product.comingSoon` + `releaseAt`): it lists with "Expected &lt;date&gt;" and a **Notify me** form in place of Add to cart. A sold-out product offers the same. One email is ever sent per request — the day the product becomes buyable — and sending it retires the row, so there is no confirmation step and nothing to unsubscribe from. Deliberately **not** called `preorder`: a pre-order is something you pay for now and receive later, which needs the checkout to take money for stock that doesn't exist yet; this is only the announcement, and the name says so. [`lib/store/availability.ts`](lib/store/availability.ts) is the one answer to "what can a visitor do with this product" — the Store card, the product page, its JSON-LD and the API all ask it, so a card can't say Sold out over a page with a buy button
- **Mailing list** — double-opt-in signup in the site footer, on `/subscribe`, and on `/shows` whenever there are no dates booked (the one place a visitor arrives wanting a date and there isn't one). A row is created the moment an address is typed in but counts as a subscriber only once the link in the confirmation email is clicked, so an address entered by somebody else never joins the list. **Nothing in the app sends campaigns** — the admin exports the confirmed rows and sends from a mail provider, which is why the CSV is the point of the module. Hardening: the signup answers identically whether the address is new, pending, already confirmed or blocked (anything else makes it an oracle for who's on the list), it's rate limited per IP and per address by a 10-minute resend cooldown so it can't be used to bomb an inbox, unsubscribe is a **POST from a button** rather than a GET (mail scanners follow every link in an email), and the unsubscribe token is never rotated so a link in an old email keeps working. Helpers in [`lib/subscribers.ts`](lib/subscribers.ts) / [`lib/subscribers-server.ts`](lib/subscribers-server.ts); the one email is [`emails/SubscribeConfirm.tsx`](emails/SubscribeConfirm.tsx)
...
- **Gigs map** — an OpenStreetMap/Leaflet map section at the foot of `/shows` (no API key or billing account required), one pin per place the band has played (title, venue, date, description, photos/videos in a click-to-open modal). Whichever event is admin-flagged **Next Event** gets a distinct yellow, gently pulsing pin (respects `prefers-reduced-motion`); every other pin is plain red. Silently omitted if no events are enabled yet — see [`components/public/EventsMap.tsx`](components/public/EventsMap.tsx)
...
- SEO/discoverability — `/sitemap.xml` (static pages, one entry per published release, and one per song that has lyrics, each with the row's own `updatedAt` as `lastModified`; `revalidate = 3600`, because prerendered-once meant a release published after the last deploy was missing from it — exactly the content whose point is being found), `/robots.txt` (disallows `/admin`, `/api`, auth pages, and personalized routes like `/cart`/`/wishlist`), per-page OpenGraph/Twitter card metadata, and JSON-LD structured data (`MusicAlbum` + `BreadcrumbList` on release pages, `Product` + `BreadcrumbList` on artwork pages, `MusicGroup` on the homepage and About, `MusicEvent` for every upcoming show on `/shows`)
...
- **Products** — price/stock/availability per artwork (V2 shop), with the same Featured/New Release/Draft quick-filter toggles as Artworks. Each product also has a **Coming soon** switch and an optional expected date, and a **N waiting** badge when people have asked to be told about it — clicking it lists them with a CSV export. There is deliberately no "email them now" button: the alert fires automatically when the product actually becomes buyable (Coming soon off, or stock back), because an email that says "it's available" has to be true when it lands. The save toast reports how many were emailed
...
- **Shows / Events** (`/admin/events`) — full CRUD for `/shows` and its map: title, venue, **city**, **date & set time**, **status** (Scheduled / Sold out / Cancelled / Postponed), **ticket link**, **price** and **price note**, **lineup** (one act per line), description, click-to-drop-pin location picker (falls back to manual lat/lng inputs without a Maps key), enable toggle, reorder, and a **Next Event** star toggle (server-enforced single flag, same transactional pattern as the Digital Museum's entry-room). Leave the date blank for a TBA show — it still lists, as upcoming. The date field carries a time: it used to be date-only, which is why every show once read "8:00 AM" on the public site (a date-only value is stored as midnight UTC, which is 8am in Manila — see `hasShowTime` in [`lib/shows.ts`](lib/shows.ts), which leaves the time off a row that has none). Photo/video upload straight to R2. Wired into **Global Search** and the sidebar
- **Lyric sync** (in **Releases**, per track) — tap-sync: play the record and press <kbd>space</kbd> as each line comes round, and the Lyrics Wall and the lyrics page follow the song exactly instead of estimating. Per-line nudge (±0.25s), clear, undo, click a timestamp to hear that line, 0.5×/0.75× playback for fast passages. **Timings are seconds into the record's audio file** (`VinylRecord.audioUrl`), not into the track — that one file per record is the only audio the site has and the only clock every consumer shares, so a release with no published vinyl has nothing to sync against and the panel says so. The timings live in the track draft and save with the release, so a sync is abandoned by cancelling the form. **`lyricTimings` and `slug` are carried by the form, not merged**: `PATCH /api/releases/[id]` deletes and recreates every track, so a payload that omitted them would destroy an afternoon's work and move every lyrics-page URL
- **Press Kit** (`/admin/press`) — the handful of fields `/press` needs that the rest of the site has no reason to store: short bio, booking name/email, photo credit, up to eight press quotes, the tech rider and a stage plot image. Everything else on that page is edited where it already lives (About, Band Members, Releases, Shows), and the page says so with links rather than offering a second place to change it. Writes through the same `PUT /api/profile` as every other Profile editor. Press quotes are one **Json column** (`Profile.pressQuotes`) rather than a table — the same call `hoverShimmer` makes: they're always read and written with the rest of the kit, there are a handful, and nothing queries or soft-deletes them, so a table plus its Trash wiring would buy nothing
- **Mailing List** (`/admin/subscribers`) — who's on the list, who hasn't confirmed, who's left, with search, a status filter and **CSV export** of whatever is on screen (the export always carries a `status` column, so a file that happens to include unconfirmed or unsubscribed rows says so rather than looking like a clean list). Read-only apart from removing a row — there is deliberately no "add subscriber" button, since an address that didn't confirm for itself has no business on the list. Removing is a **real delete, not Trash**: the row is somebody's email address, and a thirty-day recycle bin is the opposite of what a "delete my data" request means; unsubscribing is the reversible option and keeps the record so the address can't be re-added by a later import. Neither token ever reaches the browser
...
- **Trash** — soft-delete recovery for artworks, sections, products, cosplays, releases, videos, vinyls, band members, announcements, marquees, notifications, Digital Museum rooms, and shows (`deletedAt`)
- **Global Search** (⌘K / Ctrl+K, `components/admin/GlobalSearch.tsx`) — command-palette search across Artworks, Products, Orders, Sections, Rooms, Announcements, FAQs, and Shows, plus a quick-nav shortcut to every admin page
...
Blocks are managed (added or reversed) from **Settings → Blocked Emails** (`app/(admin)/admin/settings/blocked-emails/`, `app/api/admin/blocked-emails/`) — lists every `BlockedEmail` row with an **Unblock** button, and a form to block an address directly without going through an existing Gmail notification. The mailing-list signup checks the same table, and answers a blocked address with the ordinary acknowledgement and no row.
...
│   │                                # Shows, plus quick-nav to every admin page),
...
│   │                                #   SubscribeConfirm (the mailing list's only email),
...
│   ├── add-menu-item.ts             # Adds one row to the live menu bar (idempotent); a saved menu ignores DEFAULT_MENU_ITEMS
