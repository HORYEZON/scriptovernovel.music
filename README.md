...
- Dark/light theme toggle — the sun/moon pill to the right of **MENU** in the header, always on, dark by default for a first-time visitor (choice persisted in `localStorage`). The header itself is frosted glass (kalamari.arts's navbar recipe: a translucent tint over `backdrop-blur` that thickens on scroll); the admin's **Site Design → Header** colours tint it in light mode, dark mode uses ink/cream
