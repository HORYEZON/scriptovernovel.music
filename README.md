...
- SEO/discoverability — `/sitemap.xml` (static pages + every published artwork), `/robots.txt` (disallows `/admin`, `/api`, auth pages, and personalized routes like `/cart`/`/wishlist`), per-page OpenGraph/Twitter card metadata, and JSON-LD structured data (`Product` + `BreadcrumbList` on artwork pages, `MusicGroup` on the homepage and About, `MusicEvent` for every upcoming show on `/shows`)
