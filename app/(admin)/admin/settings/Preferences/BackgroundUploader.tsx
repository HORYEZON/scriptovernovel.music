// app/(admin)/admin/settings/Preferences/BackgroundUploader.tsx
"use client";
...
// Extracted from the old "Site Background" block in AboutClient.tsx —
// replaces the default background on the public site and admin login
// (see Profile.backgroundImage). Also reused for the admin-only background
// image (SiteTheme.adminBackgroundImage) via the label/helpText/revertLabel
// props below — same upload mechanics, different target column.
export function BackgroundUploader({
  value,
  onChange,
  label = "Site Background Image",
  helpText = "Replaces the default background on the public site and admin login.",
  revertLabel = "Revert to default background",
}: {
  value: string;
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          {uploading ? (
            ...
            <>
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/60 dark:bg-ink-900/60 backdrop-blur-sm">
                <Upload
                  size={20}
                  className="text-sepia dark:text-cream sm:w-[22px] sm:h-[22px]"
                />
              </div>
